    ...
    return Object.values(grouped)
        .map(item => {
            ...
            const calendarDays = observedDates.length
                ...
                : 1;
            const averageDaily = item.used / calendarDays;
            const baseForecast = averageDaily * App.forecast.days;
            const recommendedQuantity = baseForecast * (1 + App.forecast.buffer);
            const stock = stockMap[item.material] || 0;
            const toOrder = Math.max(0, recommendedQuantity - stock);
            ...
            <tbody>
                ${rows.map(row => `
                    <tr>
                        ...
                        <td>${formatNumber(row.baseForecast)}</td>
                        <td>${formatNumber(row.recommendedQuantity)}</td>
                        <td>${formatNumber(row.stock)}</td>
                        <td><strong>${formatNumber(row.toOrder)}</strong></td>
                    </tr>
                `).join('')}
            </tbody>
        </table>
...
function getCoverageRows(rows) {
    ...
    return getLatestStockByMaterial(rows).map(item => {
        ...
        const calendarDays = period
            ...
            : 1;
        const averageDaily = (usage[item.material] || 0) / calendarDays;

        return {
            material: item.material,
            stock: item.stock,
            averageDaily,
            coverageDays: averageDaily > 0
...
function getParetoData(rows) {
    ...
    const result = grouped.map(item => {
        ...
        if (cumulativeShare <= 80 || materialsFor80 === 0) {
            materialsFor80++;
        }
