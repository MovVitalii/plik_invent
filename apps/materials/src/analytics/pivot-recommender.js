...
(function initializePivotRecommender(global) {
    ...
    function addToAccumulator(accumulator, value, orderValue = null, sequence = 0) {
        ...
        const orderDate = core.parseDate(orderValue);
    ...
    function aggregate(values, aggregation) {
        ...
        values.forEach((value, index) => addToAccumulator(accumulator, value, null, index));
        ...
        rows.forEach((row, rowIndex) => {
            ...
            const columnKey = columnField
                ? configuration.timeGranularity
                    ? (core.periodKey(rawColumnValue, configuration.timeGranularity) || "(brak)")
            ...
            addToAccumulator(bucket.get(columnKey), value, orderValue, rowIndex);
            if (!totals.has(rowKey)) totals.set(rowKey, createAccumulator());
            addToAccumulator(totals.get(rowKey), value, orderValue, rowIndex);
        });
